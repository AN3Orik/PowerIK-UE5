...
void FAnimNode_PowerIK_Ground::PutEffectorsOnGround(FComponentSpacePoseContext& Output)
{
	...
	for (int i = 0; i < FootEffectors.Num(); ++i)
	{
		...
		if (Feet.Num() < i + 1)
		{
			// Can happen only in editor when feet removed
			continue;
		}
...
bool FAnimNode_PowerIK_Ground::InitializeSolverData(const FBoneContainer& RequiredBones)
{
	...
	std::vector<FPowerIKGroundFoot> ValidFoots;
	for (int i= 0; i < Feet.Num(); ++i)
	{
		...
		{
			ValidFoots.push_back(Foot);
		}
	}

	if (ValidFoots.size() < 2)
	{
		// Need at least two legs for initialize solver
		return false;
	}
	
	FootEffectors.SetNum(ValidFoots.size());
	GroundFeet.resize(ValidFoots.size());

	for (unsigned int i = 0; i < ValidFoots.size(); ++i)
	{
		const FPowerIKGroundFoot& Foot = ValidFoots[i];
